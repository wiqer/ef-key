...
public class EFKVMultipleTcpClient {

    //属性
    EventLoopGroup group ;
    Bootstrap bootstrap ;
    //private volatile int count;
    public EFKVMultipleTcpClient() {
        /*怕单线程扛不住*/
        group = new NioEventLoopGroup(1);
        StringEncoder  stringEncoder = new StringEncoder();
        StringDecoder stringDecoder=new StringDecoder();
        ByteBuf delimiterBased= Unpooled.copiedBuffer(EFConstant.DELIMITER.getBytes());
        bootstrap = new Bootstrap()
                ...
                .handler(new ChannelInitializer<SocketChannel>() {
                    ...
                    protected void initChannel(SocketChannel ch) throws Exception {
                        ...
                        pipeline.addLast(new DelimiterBasedFrameDecoder(EFConstant.MAX_LENGTH,delimiterBased));
                        pipeline.addLast("decoder",stringDecoder);
                        pipeline.addLast("encoder", stringEncoder);
                        pipeline.addLast(new IdleStateHandler(0,0,60));
                        //加入自定义的handler
                        pipeline.addLast(new EFKVMultipleTcpClientHandler());
                    }
                });
    }
        ...
        try {
            ...
            while (true) {
                ...
                for(int i=n;i<1000;i+=8){
                    EFKVMultipleTcpClientHandler.count++;
//                   //不使用StringEncoder编码器可以直接使用Buf发送
//                    ByteBuf buf= Unpooled.copiedBuffer(i+">^ω^<", CharsetUtil.UTF_8);
//                    channel.writeAndFlush(buf);
                    Msg msg =new Msg();
                    msg.setO(OperationType.CREAT);
                    msg.setT(MsgType.STRING);
                    msg.setD(new Integer(i));
                    msg.setK("aa"+i);
                    channel.writeAndFlush(FastJsonUtils.convertObjectToJSON(msg)+ EFConstant.DELIMITER);
                    channel.id();
//                    finally {
//                        buf.release();
//                    }
...
//                    }
                ...
                System.out.println("程序运行时间：" + (endTime - startTime) + "ms,GroupChatClientHandler.msgcount="
                        +EFKVMultipleTcpClientHandler.msgcount+",count="+ EFKVMultipleTcpClientHandler.count+"添加次数+"+(++counts));    //输出程序运行时间
                while (EFKVMultipleTcpClientHandler.count>EFKVMultipleTcpClientHandler.msgcount)
                Thread.sleep(1);
            }
    ...
    public  static  void  runMul(int startPort,int n){
        ...
        for (int i=0; i<n;i++) {
            ...
            multipleGroup.execute(() -> {
                try {
                    new EFKVMultipleTcpClient().run(nIndex,"10.100.16.27", startPort+nIndex);;
                } catch (Exception e) {
                    e.printStackTrace();
                }
            });
