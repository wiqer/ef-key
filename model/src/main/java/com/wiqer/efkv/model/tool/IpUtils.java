...
public final class IpUtils {
    ...
    private static volatile int cachedIntIpAddress=0;

    public static void main(String[] args) {
        System.out.println(getIp());
        System.out.println(getIntIp());
    ...
    public static int getIntIp() {
        if (0 != cachedIntIpAddress) {
            return cachedIntIpAddress;
        }
        Enumeration<NetworkInterface> netInterfaces;
        try {
            netInterfaces = NetworkInterface.getNetworkInterfaces();
        ...
        while (netInterfaces.hasMoreElements()) {
            ...
            while (ipAddresses.hasMoreElements()) {
                ...
                if (isPublicIpAddress(ipAddress)) {
                    ...
                    do {
                        --l;
                        publicIntIpAddress+= bytes[l]<<((3-l)*8);
                    } while (l>0);
                    return publicIntIpAddress;
                }
                if (isLocalIpAddress(ipAddress)) {
                    byte[] bytes  = ipAddress.getAddress();
                    int l=4;
