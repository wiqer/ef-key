package com.wiqer.efkv.client.tcp;
...
public class EFKVTcpClient {

    //属性
    EventLoopGroup group ;
    Bootstrap bootstrap ;
    //private volatile int count;
    public EFKVTcpClient() {
        /*怕单线程扛不住*/
        ...
        bootstrap = new Bootstrap()
                ...
                .handler(new ChannelInitializer<SocketChannel>() {
                    ...
                    protected void initChannel(SocketChannel ch) throws Exception {
                        ...
                        //加入自定义的handler
                        pipeline.addLast(new EFKVTcpClientHandler());
                    }
                });
    }
...
//                    }
                ...
                System.out.println("程序运行时间：" + (endTime - startTime) + "ms,GroupChatClientHandler.msgcount="
                        + EFKVTcpClientHandler.msgcount+",count="+ EFKVTcpClientHandler.count+"添加次数+"+(++counts));    //输出程序运行时间
                while (EFKVTcpClientHandler.count> EFKVTcpClientHandler.msgcount)
                Thread.sleep(1);
            }
    ...
    public  static  void  runMul(int startPort,int n){
        ...
        for (int i=0; i<n;i++) {
            ...
            multipleGroup.execute(() -> {
                try {
                    new EFKVTcpClient().run(nIndex,"10.100.16.27", startPort+nIndex);;
                } catch (Exception e) {
                    e.printStackTrace();
                }
            });
